...
const Dashboard: React.FC<DashboardProps> = ({ user, sales, expenses }) => {
  ...
  const sendExecutiveReport = () => {
    const report = `📊 *EXECUTIVE SUMMARY - NEXUS OS*%0A%0A` +
    ...
    <div className="space-y-10 animate-in fade-in">
      <header className="flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-3xl font-black text-white font-orbitron uppercase tracking-tighter">{user.officeName || 'Nexus Core'}</h2>
            <p className="text-[10px] font-orbitron text-cyber-neon tracking-widest uppercase opacity-70">Operação em Conformidade</p>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <button onClick={sendExecutiveReport} className="glass px-6 py-4 rounded-2xl border border-cyber-neon/40 flex items-center gap-3 shadow-neon-cyan hover:bg-cyber-neon hover:text-black transition-all text-cyber-neon font-orbitron text-[9px] font-black uppercase tracking-widest group">
            <Share2 size={16} className="group-hover:rotate-12 transition-transform"/> Relatório Zap
          </button>
          <div className="glass px-8 py-4 rounded-2xl border border-cyber-neon/20 flex flex-col items-center">
             <div className="text-3xl font-black digital-font text-white">{time.getHours().toString().padStart(2, '0')}:{time.getMinutes().toString().padStart(2, '0')}:{time.getSeconds().toString().padStart(2, '0')}</div>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10">
        <div className="glass p-8 rounded-3xl border border-white/5 bg-black/40 shadow-sm">
           <h3 className="text-sm font-orbitron font-black text-white mb-6 uppercase tracking-widest flex items-center gap-3"><Timer size={18} className="text-cyber-neon"/> Transações Recentes</h3>
           <div className="space-y-2">
              {sales.slice(0, 5).map(s => (
                <div key={s.id} className="flex justify-between items-center bg-white/5 p-4 rounded-xl border-l-2 border-cyber-neon hover:bg-white/10 transition-colors">
                  <div className="flex flex-col"><span className="text-[10px] font-black text-white uppercase">{s.customerName}</span><span className="text-[8px] text-gray-500">{new Date(s.date).toLocaleDateString()}</span></div>
                  <span className="text-xs font-black digital-font text-cyber-neon">R$ {s.total.toFixed(2)}</span>
                </div>
              ))}
              {sales.length === 0 && <p className="text-center py-10 opacity-20 uppercase text-[10px] tracking-widest">Sem protocolos de venda no ciclo atual.</p>}
           </div>
        </div>

        <div className="glass p-8 rounded-3xl border border-white/5 bg-black/40 shadow-sm flex flex-col">
           <h3 className="text-sm font-orbitron font-black text-white mb-8 uppercase tracking-widest flex items-center gap-3"><Package size={18} className="text-cyber-neon"/> Resumo Almoxarifado</h3>
           <div className="flex-1 flex flex-col items-center justify-center space-y-4">
              <div className="w-24 h-24 rounded-full border-4 border-cyber-neon/20 flex items-center justify-center relative">
                 ...
                 <span className="text-2xl font-black font-orbitron text-white">{sales.length}</span>
              </div>
              <p className="text-[10px] text-gray-500 uppercase tracking-widest">Saídas Processadas</p>
           </div>
        </div>
      </div>
    </div>
...
const MetricCard = ({ title, value, color, icon }: any) => (
  <div className="p-6 rounded-3xl border border-white/5 bg-black/40 card-neo shadow-sm">
    <div className="flex justify-between items-center mb-3">
      ...
      <div className={color === 'neon' ? 'text-cyber-neon' : color === 'pink' ? 'text-cyber-pink' : 'text-cyber-yellow'}>{icon}</div>
    </div>
